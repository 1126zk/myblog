...
本节实现一个支持 把多个应用的代码和数据放置到内存中，并能够执行每一个应用

在应用程序发出 sys_yeild 系统调用时，能切换应用，从而让CPU尽可能忙于每个应用的计算任务

提高任务调度的灵活性 和 CPU 的使用效率
...
1. - os/src/main.rs
   ...
   6. user/src/bin/00write_a.rs
    ...
    8. 再次返回到 os/src/task/mod.rs

        - 先将 当前任务的状态标记为 Exited
        - 再调用 run_next_task

    9. 如果所有的任务都完成了，就 panic 打印出 All applications completed!
